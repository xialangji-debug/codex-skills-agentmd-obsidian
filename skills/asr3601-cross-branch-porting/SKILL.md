...
2. Search only `%USERPROFILE%\Documents\Obsidian\CodexVault\Codex\fix-patterns` for 1-3 relevant patterns.
3. Read only matching pattern files.
4. If nothing matches, do not broaden to the whole vault.

After every completed behavior-changing port, update the canonical root-cause note
and add one concise exact-target row unless the user explicitly says not to record
it. Do not copy the full fix into a second branch-specific note.
...
6. Record the target application through `fix_memory.py upsert --note <canonical>
   --write`. Static/build-only ports remain working. Device/platform/QA evidence may
   upgrade only the target that actually passed.
7. List other active-project candidates read-only. Never switch or edit those
   branches unless the user explicitly selects them.
...
For verification and final reporting, use `asr3601-fix-closeout-reporter` when the
target repo has a usable Git/build context. Use `obsidian-fix-pattern-memory` to
update the same canonical note and exact target row. A high-confidence existing
root cause may be updated automatically; ambiguous note merges require review.
