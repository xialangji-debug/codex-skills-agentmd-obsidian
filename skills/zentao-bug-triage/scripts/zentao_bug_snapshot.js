...
let SITE_URL = process.env.ZENTAO_URL || "";
...
const LOCAL_PROJECT_MAP_PATH = path.join(
  os.homedir(),
  ".codex",
  "zentao-bug-triage",
  "project-map.local.md"
);

function resolveProjectMapPath(skillRoot) {
  if (fs.existsSync(LOCAL_PROJECT_MAP_PATH)) return LOCAL_PROJECT_MAP_PATH;
  return path.join(skillRoot, "references", "project-map.md");
}
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--site-url") args.siteUrl = next();
    else if (a === "--help" || a === "-h") {
      console.log(`Usage:
  ...
  node zentao_bug_snapshot.js --repo . --project-name "Example Firmware Project" --limit 80
  node zentao_bug_snapshot.js --repo . --project-id 100 --limit 80
  node zentao_bug_snapshot.js --repo . --product-id 200 --project-name "Example Firmware Project" --limit 80
  ...
  node zentao_bug_snapshot.js --repo . --expect-repo-name example-firmware --expect-branch example-main --current-mine-active
  node zentao_bug_snapshot.js --cleanup "%USERPROFILE%\\.codex\\zentao-bug-triage\\snapshots\\<snapshot>"

Defaults:
  With only --repo, the script reads the private local project map first, resolves the current
...
function parseProjectMap(text) {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (!current) continue;
    const sectionMatch = trimmed.match(/^(branch_contains|local_tokens|yl_device_ver_contains|zentao_names|product_names):\s*(.*)$/);
    if (sectionMatch) {
      ...
      if (sectionMatch[2]) {
        ...
        else if (section === "product_names") current.productNames.push(value);
      }
      continue;
    }
...
function resolveCurrentProject(ctx) {
  ...
  const projectMapPath = ctx.projectMapPath || resolveProjectMapPath(skillRoot);
  ...
  if (exact) {
    return {
      ...
      productName: exact.productNames[0] || exact.zentaoNames[0],
  ...
  if (uniqueNames.length === 1) {
    return {
      ...
      productName: generic[0]?.productNames?.[0] || uniqueNames[0],
...
function classify(bug) {
  ...
  const backendStatusIconIssue = hasIssue(["后台", "服务器"]) &&
    hasIssue(["图标", "透明", "绿色", "高亮", "显示"]);
  ...
  } else if (bindingTextIssue || certificationTextIssue || lockOrOrderUiIssue || pedometerTextIssue || backendStatusIconIssue) {
    category = "UI Bug";
    difficulty = "低";
    logsNeeded = false;
    canHandle = "可以先查";
...
function rowMatchesBugStatus(row, args) {
  ...
  if (requested === "unresolved") {
    return !/^(closed|已关闭|关闭)$/.test(status);
  }
  if (requested === "closed") {
    return /^(closed|已关闭|关闭)$/.test(status);
  }
...
function projectNameMatchesExactly(value, projectName) {
  const product = normalizeText(value || "");
  const project = normalizeText(projectName || "");
  return Boolean(product && project && product === project);
}

function mappedProductName(args) {
  return args.productName || args.projectName || "";
}
...
function bugFromRow(row, args) {
  ...
  const bug = {
    ...
    url: row.href,
    title: row.title || "",
    product: row.product || args.productName || args.projectName || "",
    bugType: row.bugType || "",
...
function markdownReport(ctx, bugs) {
  ...
  lines.push("");
  lines.push("| Bug（ID + 标题） | 产品/项目 | 状态 | 严重/优先 | 创建/修改/解决 | 激活 | 详情 | 分类 | 难度 | 记忆命中 | 修复资格 | 本轮处理 | 附件 | 描述摘要 | 建议/原因 |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |");
  for (const b of bugs) {
    ...
    lines.push(`| ${bugDisplayLabel(b)} | ${(b.product || "").replace(/\|/g, "/")} | ${b.status || ""} | ${b.severity || ""}/${b.priority || ""} | ${dates} | ${activationText} | ${detailText} | ${b.category} | ${b.difficulty} | ${compactMemoryText(b)} | ${compactEligibilityText(b)} | ${handling} | ${attachmentText} | ${stepsSummary} | ${advice} |`);
  }
  return lines.join("\n");
}
...
function chatSummaryReport(ctx, bugs, snapshotDir) {
  ...
  lines.push("");
  lines.push("| Bug（ID + 标题） | 类型 | 记忆命中 | 修复资格 | 处理建议 | 附件 |");
  lines.push("| --- | --- | --- | --- | --- | --- |");
  for (const bug of bugs) {
    lines.push(`| ${bugDisplayLabel(bug)} | ${compactCategory(bug)} | ${compactMemoryText(bug)} | ${compactEligibilityText(bug)} | ${compactHandlingText(bug)} | ${compactAttachmentText(bug)} |`);
  }
  return lines.join("\n");
}
...
function workItemsReport(ctx, bugs, snapshotDir) {
  ...
  for (const bug of workItems) {
    ...
    lines.push(`## ${bugDisplayLabel(bug)}`);
    lines.push("");
...
function ignoredItemsReport(ctx, bugs, snapshotDir) {
  ...
  lines.push("");
  lines.push("| Bug（ID + 标题） | 记忆命中 | 修复资格 | 处理 | 分类 | 原因 | 建议 |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- |");
  for (const bug of ignoredItems) {
    lines.push(`| ${bugDisplayLabel(bug)} | ${compactMemoryText(bug)} | ${compactEligibilityText(bug)} | ${bug.handlingLabel || ""} (${bug.handlingAction || ""}) | ${bug.category || ""} | ${(bug.handlingReason || "").replace(/\|/g, "/")} | ${(bug.advice || "").replace(/\|/g, "/")} |`);
  }
  lines.push("");
...
function assignedFallbackRows(rows, projectName, limit, detailLimit) {
  const matching = rows.filter((row) => projectNameMatchesExactly(row.product, projectName));
  const rowsWithProduct = rows.filter((row) => normalizeText(row.product || ""));
  if (rowsWithProduct.length) return matching.slice(0, limit);
...
async function main() {
  ...
  SITE_URL = (args.siteUrl || loadSiteUrl()).replace(/\/+$/, "");
  ...
  if (projectResolution.mode === "project") {
    ...
    args.productName = projectResolution.productName || projectResolution.projectName;
    ctx.productName = args.productName;
  ...
  try {
    ...
    if (!args.ids.length && !args.assigned && args.projectId && args.productId && rows.length === 0) {
      rows = await collectRowsFromStart(page, productBrowseUrl, args, { applyProjectFilter: true, assignedIdSet });
      productFallback = rows.length > 0;
      if (productFallback) {
        detailLimit = rows.length;
        ctx.fetchMode = "current-product-fallback";
        ctx.projectResolveReason = `${ctx.projectResolveReason || "project mapping"}; project/list returned 0 rows, fetched mapped product ${args.productId}`;
      }
    }
    ...
    if (!args.ids.length && !args.assigned && args.projectName && rows.length === 0) {
      ...
      rows = assignedFallbackRows(assignedRows, mappedProductName(args), args.limit, args.detailLimit);
    ...
    if (productFallback) {
      bugs = bugs.filter((bug) => projectNameMatchesExactly(bug.product, mappedProductName(args)));
    }
    if (assignedProjectFallback) {
      bugs = bugs.filter((bug) => projectNameMatchesExactly(bug.product, mappedProductName(args)));
    }
