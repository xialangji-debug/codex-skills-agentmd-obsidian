...
function detectVerification(markdown) {
  const managed = decodeManagedState(markdown);
  if (managed) {
    const reference = (managed.targets || []).find((target) => target.target_id === managed.reference_target);
    if (reference && ["device_verified", "platform_verified", "qa_verified"].includes(reference.verification)) return "verified";
    if (reference || (managed.targets || []).some((target) => target.verification === "needs_review")) return "unverified";
  }
...
function decodeManagedState(markdown) {
  const match = markdown.match(/<!--\s*codex-fix-state-json:\s*([A-Za-z0-9_=-]+)\s*-->/);
  if (!match) return null;
  try {
    const state = JSON.parse(Buffer.from(match[1], "base64url").toString("utf8"));
    return state.schema_version === 2 ? state : null;
  } catch {
    return null;
  }
}
