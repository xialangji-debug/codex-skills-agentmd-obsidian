...
Compact exact-target reconciliation state:
...
```text
%USERPROFILE%\Documents\Obsidian\CodexVault\Codex\projects\zentao\
```
...
It never deletes `state\`. Removing one target state file resets the comparison
baseline for that target, so the next fetch cannot infer historical transitions.
