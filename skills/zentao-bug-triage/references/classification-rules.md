...
| Bug（ID + 标题） | 类型 | 记忆命中 | 修复资格 | 处理建议 | 附件 |
| --- | --- | --- | --- | --- | --- |
| 3526 计算机界面向右滑动会有点卡顿 | UI | 未命中 | 需先查代码 | 可查 | 无 |

Use one ASCII space between the numeric ID and the full one-line title. Do not add `#`, split ID/title into separate user-facing columns, or show a naked ID. If the title is unavailable, use `<数字ID> 标题未获取`. Machine-facing JSON fields, URLs, filenames, logs, and CLI arguments keep pure numeric IDs.
