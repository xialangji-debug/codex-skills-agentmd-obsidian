...
```powershell
& "$env:USERPROFILE\.codex\skills\codex-clash-proxy\scripts\run-with-clash-proxy.ps1" -CommandLine "<command> <args>"
```
...
```powershell
& "$env:USERPROFILE\.codex\skills\codex-clash-proxy\scripts\run-with-clash-proxy.ps1" -PreferAutoGroup -CommandLine "git clone https://github.com/example/repo.git"
```
...
```powershell
& "$env:USERPROFILE\.codex\skills\codex-clash-proxy\scripts\run-with-clash-proxy.ps1" -PreferAutoGroup -CommandLine "git clone https://github.com/owner/repo.git"
```

Download a file:

```powershell
& "$env:USERPROFILE\.codex\skills\codex-clash-proxy\scripts\run-with-clash-proxy.ps1" -CommandLine "curl.exe -L -o tool.zip https://example.com/tool.zip"
```

Install Python dependencies:

```powershell
& "$env:USERPROFILE\.codex\skills\codex-clash-proxy\scripts\run-with-clash-proxy.ps1" -CommandLine "python -m pip install -r requirements.txt"
```

Only start Clash and verify the local proxy port:

```powershell
& "$env:USERPROFILE\.codex\skills\codex-clash-proxy\scripts\run-with-clash-proxy.ps1"
```
