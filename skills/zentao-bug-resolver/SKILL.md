...
- Before preview and submit, read the detail-page `所属产品` and require an exact match with `.codex-project/zentao.md`. Missing or prefix-only matches are failures. Use `--allow-product-mismatch` only for an explicitly requested corrective reactivation, never for normal resolution.
...
- Treat `已解决` as QA pending. After a successful fixed-resolution submit, the
  resolver script sends a local `zentao_resolved` event to the exact linked
  fix-pattern target; it never upgrades that target to QA verified.
...
- If a bug from the wrong product was accidentally marked `已解决`, use `--reactivate-resolved` in preview mode first, then submit. This mode only restores `已解决 -> 激活`, refuses other source states, and never runs the resolve form.
...
Correct an accidental cross-product resolution:
...
7. Report the local memory-event count. An unmatched event means the Bug has no
   exact canonical target row yet; do not guess or attach it by title alone.

The service URL is private local configuration. Set `ZENTAO_URL` or store it in
`%USERPROFILE%\.codex\secrets\zentao-bug-triage\zentao.url`. Never put a live
company URL in this Skill or a reusable repository.

## Resources

- `scripts/zentao_bug_resolver.js`: opens configured Zentao resolve forms, validates solution/build/assignee/comment choices, writes preview reports, and submits only with `--submit`.
