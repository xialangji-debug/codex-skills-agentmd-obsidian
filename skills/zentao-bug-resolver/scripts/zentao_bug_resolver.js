...
const DEFAULT_SITE_URL = process.env.ZENTAO_URL || "";

function loadSiteUrl() {
  if (DEFAULT_SITE_URL) return DEFAULT_SITE_URL.replace(/\/+$/, "");
  const localPath = path.join(os.homedir(), ".codex", "secrets", "zentao-bug-triage", "zentao.url");
  if (fs.existsSync(localPath)) {
    const value = fs.readFileSync(localPath, "utf8").trim().replace(/\/+$/, "");
    if (value) return value;
  }
  throw new Error("Missing Zentao URL. Set ZENTAO_URL or create ~/.codex/secrets/zentao-bug-triage/zentao.url");
}
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--reactivate-resolved") args.reactivateResolved = true;
    else if (a === "--activate-comment") {
      args.activateComment = next();
      args.activateCommentExplicit = true;
    }
    else if (a === "--expect-product") args.expectedProduct = next();
    else if (a === "--allow-product-mismatch") args.allowProductMismatch = true;
...
Defaults:
  ...
  --reactivate-resolved only restores mistakenly resolved bugs to 激活 and performs no resolve action.
  The current project's Zentao product is checked exactly before preview and submit.
  Use --allow-product-mismatch only for an explicit corrective reactivation.
...
function variantValue(repo, label) {
  const variant = path.join(repo, ".codex-project", "variant.md");
...
function memoryTargetContext(ctx, expectedProduct) {
  const remote = run(["git", "remote", "get-url", "origin"], ctx.repo);
  const repoId = identityHash(remote || ctx.repo, 12);
...
function applyMemoryEvents(outDir, ctx, args, items, results) {
  if (!args.submit || args.reactivateResolved) return { events: 0, matched: 0, skipped: "no submitted fixed resolution" };
  const target = memoryTargetContext(ctx, args.expectedProduct || "");
  const itemById = new Map(items.map((item) => [`${item.id}`, item]));
  const events = results.filter((result) => {
    const item = itemById.get(`${result.id}`);
    return result.submitted && result.finalStatus === "已解决" && item && item.resolution === "fixed";
  }).map((result) => ({
    event: "zentao_resolved",
    bug_id: `${result.id}`,
    target_id: target.target_id,
    evidence: `Bug #${result.id}: resolver submit reached 已解决; QA pending`,
    observed_at: new Date().toISOString(),
  }));
  if (!events.length) return { events: 0, matched: 0 };
  const eventPath = path.join(outDir, "resolve-memory-events.json");
  fs.writeFileSync(eventPath, JSON.stringify({ schema_version: 1, context: target, events }, null, 2), "utf8");
  const script = path.join(os.homedir(), ".codex", "skills", "obsidian-fix-pattern-memory", "scripts", "fix_memory.py");
  if (!fs.existsSync(script)) return { events: events.length, matched: 0, skipped: "fix_memory.py missing", eventPath };
  const python = process.env.PYTHON || "python";
  const applied = cp.spawnSync(python, ["-X", "utf8", script, "apply-events", "--events", eventPath, "--write"], {
    encoding: "utf8",
    windowsHide: true,
  });
  if (applied.status !== 0) throw new Error(applied.stderr.trim() || applied.stdout.trim() || "Memory event update failed");
  let summary = { output: applied.stdout.trim() };
  try { summary = JSON.parse(applied.stdout); } catch {}
  return { ...summary, eventPath };
}
...
function productNamesEqual(left, right) {
    ...
    for (const frame of page.frames()) {
      const info = await frame.evaluate((id) => {
        ...
        const fieldValue = (label) => {
          const exactIndex = lines.indexOf(label);
          if (exactIndex >= 0) return lines[exactIndex + 1] || "";
          const inline = lines.find((line) => line.startsWith(label) && line.length > label.length);
          return inline ? inline.slice(label.length).trim() : "";
        };
        const titleMatch = (document.title || "").match(/^BUG #\d+\s+.+?\s+-\s+(.+?)\s+-\s+禅道$/);
        return {
          status: match ? match[1].trim() : "",
          title: idIndex >= 0 ? (lines[idIndex + 1] || "") : "",
          product: fieldValue("所属产品") || (titleMatch ? titleMatch[1].trim() : ""),
        };
      }, `${bugId}`).catch(() => ({ status: "", title: "", product: "" }));
...
async function processBug(page, args, item) {
  ...
  if (args.expectedProduct && !args.allowProductMismatch &&
      !productNamesEqual(beforeInfo.product, args.expectedProduct)) {
    throw new Error(`Bug #${item.id} product mismatch: expected "${args.expectedProduct}", got "${beforeInfo.product || "(empty)"}".`);
  }

  if (args.reactivateResolved) {
    if (beforeInfo.status === "激活") {
      return {
        id: item.id, title: item.title || beforeInfo.title,
        requestedResolution: "reactivate", resolution: { value: "reactivate", label: "重新激活" },
        build: { value: "", label: "" }, assignee: { value: "", label: "", preserved: true },
        beforeStatus: beforeInfo.status, activated: false, activationPlanned: false,
        activationCommentLength: 0, skipped: true, skipReason: "already-active",
        commentEmpty: false, postedCommentLength: 0, submitted: false,
        finalStatus: beforeInfo.status, formData: [], product: beforeInfo.product,
      };
    }
    if (beforeInfo.status !== "已解决") {
      throw new Error(`Bug #${item.id} cannot be reactivated from status "${beforeInfo.status}"; expected 已解决.`);
    }
    if (!args.submit) {
      return {
        id: item.id, title: item.title || beforeInfo.title,
        requestedResolution: "reactivate", resolution: { value: "reactivate", label: "重新激活" },
        build: { value: "", label: "" }, assignee: { value: "", label: "", preserved: true },
        beforeStatus: beforeInfo.status, activated: false, activationPlanned: true,
        activationCommentLength: 0, skipped: true, skipReason: "dry-run: resolved bug would be reactivated with --submit",
        commentEmpty: false, postedCommentLength: 0, submitted: false,
        finalStatus: beforeInfo.status, formData: [], product: beforeInfo.product,
      };
    }
    ...
    return {
      ...
      requestedResolution: "reactivate", resolution: { value: "reactivate", label: "重新激活" },
      build: { value: "", label: "" }, assignee: { value: "", label: "", preserved: true },
      beforeStatus: beforeInfo.status, activated: true, activationPlanned: false,
      activationCommentLength: activation.postedCommentLength, skipped: false, skipReason: "",
      commentEmpty: false, postedCommentLength: activation.postedCommentLength, submitted: true,
      finalStatus: activation.status, formData: [], product: beforeInfo.product,
    };
  }
...
async function main() {
  ...
  try {
    ...
    for (const item of items) {
      const action = args.reactivateResolved
        ? (args.submit ? "Reactivating" : "Previewing reactivation for")
        : (args.submit ? "Submitting" : "Previewing");
      console.log(`${action} Bug #${item.id} (${args.reactivateResolved ? "reactivate" : item.resolution})`);
  ...
  const memory = applyMemoryEvents(outDir, ctx, args, items, results);

  console.log(`Saved: ${jsonPath}`);
  console.log(`Saved: ${mdPath}`);
  console.log(`Memory events: ${memory.events || 0}; matched: ${memory.matched || 0}`);
