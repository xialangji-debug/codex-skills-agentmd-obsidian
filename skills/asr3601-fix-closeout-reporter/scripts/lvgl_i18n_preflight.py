...
UNSPECIFIED_LANGUAGES = {"", "auto", "unknown", "unspecified"}
CHINESE_LANGUAGES = {"zh", "zh-cn", "zh-hans", "cn"}
        ...
        if not width_lines:
            findings.append(
                Finding(
                    ...
                    rel,
                    index + 1,
                    f"{match.group(1)} 设置 long mode，但附近没有显式 width/size；多语言文本可能被裁切。",
                )
            )
            ...
            findings.append(
                Finding(
                    ...
                    rel,
                    index + 1,
                    f"{match.group(1)} 在设置 width/size 前启用 long mode；LVGL v7 应先定宽再设 long mode。",
                )
            )
    return findings
        ...
        if re.search(rf"\b(?:snprintf|sprintf)\s*\(\s*{re.escape(name)}\b", window):
            findings.append(
                Finding(
                    ...
                    rel,
                    index + 1,
                    f"格式化缓冲区 {name}[{size}] 小于 64 字节；第三方语言组合文本需核对最坏长度和结尾空字符。",
                )
            )
    return findings
...
def text_findings(rel: str, lines: list[str], language: str) -> list[Finding]:
    findings: list[Finding] = []
    calendar = re.compile(
        r"WATCH_TEXT_ID_CALENDAR_(?:SUNDAY|MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY|SATURDAY)"
    )
    string_literal = re.compile(r'"([^"\\]*(?:\\.[^"\\]*)*)"')
    normalized_language = language.strip().lower()
    check_hardcoded_cjk = (
        normalized_language not in UNSPECIFIED_LANGUAGES
        and normalized_language not in CHINESE_LANGUAGES
    )
    for index, line in enumerate(lines):
        if "sprintf(" in line and "snprintf(" not in line:
            findings.append(
                Finding(
                    ...
                    rel,
                    index + 1,
                    "UI/语言路径使用 sprintf；长翻译可能溢出，改用带 sizeof 上限的 snprintf。",
                )
            )
        if calendar.search(line):
            for value in string_literal.findall(line):
                if len(value) > 4:
                    findings.append(
                        Finding(
                            "medium",
                            ...
                            rel,
                            index + 1,
                            f"日历星期文本 {value!r} 较长；固定七列界面应使用语言认可的短写。",
                        )
                    )
        if (
            check_hardcoded_cjk
            and re.search(r"lv_label_set_(?:text|text_fmt)", line)
            ...
            findings.append(
                Finding(
                    ...
                    rel,
                    index + 1,
                    f"目标语言 {language} 的 UI 路径出现硬编码中文；确认是否应改用 text ID。",
                )
            )
    return findings
...
def scan_file(repo: Path, path: Path, language: str) -> list[Finding]:
    ...
    return (
        label_width_findings(rel, lines)
        + buffer_findings(rel, lines)
        + text_findings(rel, lines, language)
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scan changed or selected LVGL source files for localization/long-text risks."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Extra file or directory, relative to repo; repeatable.",
    )
    parser.add_argument(
        "--language",
        default="unspecified",
        help="Known target locale for hard-coded-text checks. Default: unspecified.",
    )
    parser.add_argument(
        "--resolution", default="240x240", help="Informational target resolution."
    )
    parser.add_argument(
        "--strict", action="store_true", help="Return nonzero when high-severity findings exist."
    )
...
def main() -> int:
    ...
    unique = sorted(set(candidates), key=lambda path: path.as_posix().lower())
    findings: list[Finding] = []
    for path in unique:
    ...
    if findings:
        ...
        for item in sorted(
            findings,
            key=lambda finding: (
                {"high": 0, "medium": 1}.get(finding.severity, 2),
                finding.path,
                finding.line,
                finding.rule,
            ),
