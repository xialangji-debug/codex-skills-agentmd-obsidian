...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify an ASR360x firmware fix and collect a non-destructive closeout snapshot."
    )
    ...
    parser.add_argument(
        "--skip-i18n-preflight",
        action="store_true",
        help="Skip automatic LVGL i18n/long-text scan.",
    )
    parser.add_argument(
        "--i18n-path",
        action="append",
        default=[],
        help="Extra UI/language file or directory for i18n preflight; repeatable.",
    )
    parser.add_argument(
        "--i18n-language",
        default="unspecified",
    ...
    parser.add_argument(
        "--i18n-resolution",
        default="240x240",
        help="Target display resolution for the preflight report.",
    )
    parser.add_argument(
        "--i18n-strict",
        action="store_true",
        help="Treat high-severity i18n findings as verification failures.",
    )
...
def main() -> int:
    ...
    checks: list[tuple[str, list[str] | str, bool, int, bool]] = [
        ...
        ("git diff --cached --check", ["git", "diff", "--cached", "--check"], False, 120, True),
    ]
