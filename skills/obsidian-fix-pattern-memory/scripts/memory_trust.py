...
def set_frontmatter_field(text: str, key: str, value: str) -> str:
    ...
    if not frontmatter:
        raise SystemExit("Fix-pattern note is missing YAML frontmatter")
    yaml = frontmatter.group(1)
    pattern = re.compile(rf"^{re.escape(key)}:\s*.*$", re.M)
    replacement = f"{key}: {value}"
    if pattern.search(yaml):
        yaml = pattern.sub(replacement, yaml, count=1)
    else:
        yaml = yaml.rstrip() + "\n" + replacement
...
def main() -> int:
    ...
    if args.action == "verify":
        text = set_frontmatter_field(text, "trust", "verified")
        text = set_frontmatter_field(text, "verified_at", today)
        ...
        text = set_frontmatter_field(text, "trust", "working")
        text = set_frontmatter_field(text, "updated", today)
        for key, value in [
            ("验证状态", "待复核"),
            ("可信度", "待复核"),
